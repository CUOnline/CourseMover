...
namespace Canvas.Clients
{
    public class CoursesClient : ClientBase
    {
        ...
        public async Task<Course> MoveCourse(string courseId, string canvasAccountId)
        {
            var course = await Get<Course>(courseId);
            course.AccountId = canvasAccountId;
            
            ApiPath = ApiController + $@"/{courseId}?course[account_id]={canvasAccountId}";
            return await ExecutePut<Course>(ApiPath, course);
        }
    }
}
